 ...
 * @return  array
 */
function get_categories_tree($cat_id = 0, $carid)
{
    if ($cat_id > 0)
    {
    ...
    if ($GLOBALS['db']->getOne($sql) || $parent_id == 0)
    {
        ...
        foreach ($res AS $row)
        {
            if ($row['is_show'])
            {
                ...
                {
                    $cat_arr[$row['cat_id']]['cat_id'] = get_child_tree($row['cat_id'], $carid);
                }
            }
        }
    }
...
function get_child_tree($tree_id = 0, $carid)
{
    $three_arr = array();
   ...
   " FROM " . $GLOBALS['ecs']->table('goods') . " AS g " ." WHERE ".getcargoods ($carid).
   " GROUP BY cat_id";
...
function get_top10($cats = '', $carid)
{
    ...
    $sql = 'SELECT g.goods_id, g.goods_name, g.shop_price, g.goods_thumb, SUM(og.goods_number) as goods_number ' .
           ...
           "WHERE ".getcargoods ($carid)." AND g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 $where $top10_time " ;
    //判断是否启用库存，库存数量是否大于0
 ...
 * @return  array
 */
function get_recommend_goods($type = '', $cats = '', $carid)
{
    ...
    if (empty($type_goods[$type]))
    {
        ...
        if ($data === false)
        {
            $sql = 'SELECT g.goods_id, g.is_best, g.is_new, g.is_hot, g.is_promote, b.brand_name,g.sort_order ' .
               ...
               ' WHERE '.getcargoods ($carid).' AND g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 AND (g.is_best = 1 OR g.is_new =1 OR g.is_hot = 1)'.
        ...
        $sql = 'SELECT g.goods_id, g.goods_name, g.goods_name_style, g.market_price, g.shop_price AS org_price, g.promote_price, ' .
                "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, "."IF(r.comment_rank, count(*),0) AS comment_count, ".
                "promote_start_date, promote_end_date, g.goods_brief, g.goods_thumb, g.goods_img, RAND() AS rnd " .
                'FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g ' .' LEFT JOIN  '. $GLOBALS['ecs']->table('comment') .' AS r '.
   'ON r.id_value = g.goods_id AND comment_type = 0 AND r.parent_id = 0 AND r.status = 1 ' .
        ...
        $sql .= ' WHERE '.getcargoods($carid).' AND g.goods_id ' . db_create_in($type_merge);
        $sql .= ' group by g.goods_id ORDER BY g.sort_order, g.last_update DESC';
 ...
 * @return  array
 */
function get_promote_goods($cats = '', $carid)
{
    $time = gmtime();
    $order_type = $GLOBALS['_CFG']['recommend_order'];
    ...
    $sql = 'SELECT g.goods_id, g.goods_name, g.goods_name_style, g.market_price, g.shop_price AS org_price, g.promote_price, ' .
            ...
            "LEFT JOIN " . $GLOBALS['ecs']->table('member_price') . " AS mp ".
                "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' ".
            'WHERE '.getcargoods ($carid).' AND g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 ' .
 ...
 * @return  array
 */
function get_category_recommend_goods($type = '', $cats = '', $brand = 0, $min =0,  $max = 0, $ext='', $carid)
{
    ...
    $sql =  'SELECT g.goods_id, g.goods_name, g.goods_name_style, g.market_price, g.shop_price AS org_price, g.promote_price, ' .
            ...
            "LEFT JOIN " . $GLOBALS['ecs']->table('member_price') . " AS mp ".
                    "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' ".
            'WHERE '.getcargoods ($carid).' AND g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 ' . $brand_where . $price_where . $ext;
    $num = 0;
 ...
 * @return  array
 */
function get_same_attribute_goods($attr, $carid)
{
    $lnk = array();
    ...
    if (!empty($attr))
    {
        foreach ($attr['lnk'] AS $key => $val)
        {
            ...
            $sql = 'SELECT g.goods_id, g.goods_name, g.goods_thumb, g.goods_img, g.shop_price AS org_price, ' .
                    ...
                    "LEFT JOIN " . $GLOBALS['ecs']->table('member_price') . " AS mp ".
                        "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' ".
                    "WHERE ".getcargoods ($carid)." AND a.attr_id = '$key' AND g.is_on_sale=1 AND a.attr_value = '$val[value]' AND g.goods_id <> '$_REQUEST[id]' " .
 ...
 * @return  array
 */
function assign_cat_goods($cat_id, $num = 0, $from = 'web', $order_rule = '', $carid)
{
    ...
    $sql = 'SELECT g.goods_id, g.goods_name, g.market_price, g.shop_price AS org_price, ' .
            ...
            "LEFT JOIN " . $GLOBALS['ecs']->table('member_price') . " AS mp ".
                    "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' ".
            'WHERE '.getcargoods ($carid).' AND g.is_on_sale = 1 AND g.is_alone_sale = 1 AND '.
 ...
 * @return  void
 */
function assign_brand_goods($brand_id, $num = 0, $cat_id = 0,$order_rule = '', $carid)
{
    $sql =  'SELECT g.goods_id, g.goods_name, g.market_price, g.shop_price AS org_price, ' .
                "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, ".
            ...
            "LEFT JOIN " . $GLOBALS['ecs']->table('member_price') . " AS mp ".
                    "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' ".
            "WHERE ".getcargoods ($carid)." AND g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 AND g.brand_id = '$brand_id'";
...
//获取指定车型下的所有商品
function getcargoods ($carid){
$sql = "SELECT cb_name, csys_id FROM ".$GLOBALS['ecs']->table('ctype')." WHERE id = '$carid'"; 
$res =$GLOBALS['db']-> GETROW($sql);
$sql = "SELECT goods_id FROM ".$GLOBALS['ecs']->table('goodscars')." WHERE ctype = '$carid' OR (csys = '$res[csys_id]' AND ctype = 0) OR (cbrand = '$res[cb_name]' AND csys = 0) OR is_common = 1"; 
$gid =$GLOBALS['db']-> getCol($sql);
return db_create_in($gid, 'g.goods_id');
}
