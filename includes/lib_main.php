...
//获得当前用户车型
function getcid($carid){
if($carid){
	$sql = "SELECT id FROM " .$GLOBALS['ecs']->table('ctype'). " WHERE id = $carid";
	$cid =  $GLOBALS['db']->getOne($sql);
}else{
	 $sql = "SELECT ctype FROM " .$GLOBALS['ecs']->table('usercars'). " WHERE uid = '$_SESSION[user_id]' AND defaultcar = 1";
	 $cid =  $GLOBALS['db']->getOne($sql);
}
return $cid;
}
...
function assign_template($ctype = '', $catlist = array())
{
    ...
    $smarty->assign('searchkeywords', $searchkeywords);
		...
		$cid = getcid($_GET[cid]);
		$sql = "SELECT ctype_name FROM".$GLOBALS['ecs']->table('ctype')."WHERE id = '$cid'";
		$icar =  $GLOBALS['db']->getOne($sql);
		$smarty->assign('mycar', $icar);
