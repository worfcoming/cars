    ...
    switch ($app)
    {
        case 'category':
            ...
            else
            {
                ...
                {
                    $uri = 'category.php?id=' . $cid . '&amp;cid='.$carid;
                    if (!empty($bid))
                    {
                        $uri .= '&amp;brand=' . $bid;
                    }
        ...
        case 'goods':
            ...
            {
                $uri = $rewrite ? 'goods-' . $gid . 'cid-'.$carid : 'goods.php?id=' . $gid . '&amp;cid='.$carid;
            }
        ...
        case 'brand':
            ...
            else
            {
                ...
                {
                    $uri = 'brand.php?id=' . $bid . '&amp;cid='.$carid;
                    if (!empty($cid))
                    {
                        $uri .= '&amp;cat=' . $cid;
                    }
        ...
        case 'article_cat':
            ...
            else
            {
                if ($rewrite)
                {
                    $uri = 'article_cat-' . $acid. 'cid-'.$carid;
                    if (!empty($page))
                    {
                        $uri .= '-' . $page;
                    }
                ...
                {
                    $uri = 'article_cat.php?id=' . $acid . '&amp;cid='.$carid;
                    if (!empty($page))
                    {
                        $uri .= '&amp;page=' . $page;
                    }
        ...
        case 'article':
            ...
            {
                $uri = $rewrite ? 'article-' . $aid. 'cid-'.$carid : 'article.php?id=' . $aid . '&amp;cid='.$carid;
            }
...
//取得用户车型
function getmycar($user_id){
	$sql="SELECT u.*, s.csys_name, t.ctype_name, b.cbrand_name FROM"  .$GLOBALS['ecs']->table('usercars'). " AS u". " LEFT JOIN " .$GLOBALS['ecs']->table('cbrand')." AS b ON u.cbrand = b.id"  . " LEFT JOIN " .$GLOBALS['ecs']->table('csys')." AS s ON u.csys = s.id" . " LEFT JOIN ". $GLOBALS['ecs']->table('ctype')." AS t ON u.ctype = t.id"." WHERE u.uid = '$user_id'";
	$res=$GLOBALS['db']->getall($sql);
	return $res;
}
//取得所有车品牌
function getcbrand(){
	$sql = "SELECT  *  FROM  ".$GLOBALS['ecs']->table('cbrand')." WHERE cbrand_sort = '1'  ORDER BY alpha ASC"; 
	$res =$GLOBALS['db']-> GETALL($sql);
	return $res;
}
