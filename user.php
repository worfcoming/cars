...
/* 显示页面的action列表 */
$ui_arr = array('register', 'login', 'profile', 'icar', 'add_icar', 'edit_icar', 'delicar', 'order_list', 'order_detail', 'address_list', 'collection_list',
...
{
    include_once(ROOT_PATH .'includes/lib_clips.php');
	$smarty->assign('action',     'icar');
    $smarty->assign('icar',        getmycar($user_id));
	$smarty->assign('cb',           getcbrand());
    $smarty->display('user_clips.dwt');
}
...
{
    include_once(ROOT_PATH .'includes/lib_clips.php');
	$sql="SELECT u.*, s.csys_name, t.ctype_name FROM"  .$ecs->table('usercars'). " AS u" . " LEFT JOIN " .$ecs->table('csys')." AS s ON u.csys = s.id" . " LEFT JOIN ". $ecs->table('ctype')." AS t ON u.ctype = t.id"." WHERE u.id = '$_GET[id]'";
	$res=$db->getrow($sql);
	$sql = "SELECT * FROM ".$ecs->table('csys')."WHERE cb_name = '$res[cbrand]'";
	$csys = $db->getall($sql);
	$sql = "SELECT * FROM ".$ecs->table('ctype')."WHERE csys_id = '$res[csys]'";
	$ctype = $db->getall($sql);
	
	$smarty->assign('csys',     $csys);
	$smarty->assign('ctype',     $ctype);
	$smarty->assign('icar',     $res);
	$smarty->assign('action',     'edit_icar');
	$smarty->assign('cb',           getcbrand());
    $smarty->display('user_clips.dwt');
}
if ($action == 'edit_done')
{
	...
	if($_POST['ctype_id'] != $_POST['oldtype']){
	$sql = 'SELECT id FROM ' . $ecs->table('usercars') . "WHERE `ctype`='$_POST[ctype_id]' AND `uid`='" . $_SESSION['user_id'] . "'";
	$res =  $db->getone($sql);
	}
	if ($res){
		show_message('您已经拥有这款车型了');
	}else{
		if ($_POST['defaultcar'] == 1)
		{
		 	$sql = 'UPDATE ' . $ecs->table('usercars') . " SET `defaultcar`='0'  WHERE `defaultcar`='1' AND `uid`='" . $_SESSION['user_id'] . "'";
		  	$db->query($sql);
		}
			$sql = 'UPDATE '. $ecs->table('usercars') . " SET cbrand = '$_POST[cb_name]', csys = '$_POST[csys_id]', ctype = '$_POST[ctype_id]', defaultcar = '$_POST[defaultcar]' WHERE id = '$_POST[icarid]'";
			$db->query($sql);
			header("Location: user.php?act=icar");
	}
...
if ($action == 'add_icar')
{
	...
	$sql = 'INSERT INTO '. $ecs->table('usercars') . " (`uid`, `cbrand`, `csys`, `ctype`, `defaultcar`) VALUES('$_SESSION[user_id]','$_POST[cb_name]','$_POST[csys_id]', '$_POST[ctype_id]', '$_POST[defaultcar]')";
    $db->query($sql);
	header("Location: user.php?act=icar");
	}
...
{
	$id = $_POST['cid'];
	$sql = "SELECT count(*) FROM". $ecs->table('usercars')."WHERE `uid`='" . $_SESSION['user_id'] . "'";
    $rl = $db->getone($sql);
	$sql = "SELECT defaultcar FROM". $ecs->table('usercars')."WHERE `id`='$id'";
    $rel = $db->getone($sql);
	if ($rl == 1 || $rel == 1 ){
	$res = 0;
	$json = new JSON;
	...
	$sql = "DELETE FROM " . $ecs->table('usercars') .
            " WHERE id = '$id'";
    $res = $db->query($sql);
	$json = new JSON;
	echo $json->encode($res);
...
elseif ($action == 'act_register')
{
    ...
    else
    {
        ...
        if (register($username, $password, $email, $other) !== false)
        {
            ...
            if (!empty($_POST[ctype_id]))
            {
                $sql = 'INSERT INTO '. $ecs->table('usercars') . " (`uid`, `cbrand`, `csys`, `ctype`) VALUES('$_SESSION[user_id]','$_POST[cb_name]','$_POST[csys_id]', '$_POST[ctype_id]')";
                $db->query($sql);
            }
            /* 判断是否需要自动发送注册邮件 */
